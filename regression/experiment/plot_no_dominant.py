...
def identify_experiment_type(pickle_path):
    """Identify experiment type from filename/path"""
    path_str = str(pickle_path).lower()
    filename = Path(pickle_path).name.lower()
    
    # Check for feature and combined heterogeneity
    if any(keyword in path_str for keyword in ['feature', 'combined']) and 'heterogen' in path_str:
        return 'feature_combined'
    # Check for parameter, target, and taper
    elif any(keyword in path_str for keyword in ['param', 'target']):
        return 'parameter_target'
    
    elif any(keyword in path_str for keyword in ['taper']):
        return 'taper'
    
    # Check for progressive (fallback)
    elif 'progressive' in path_str:
        return 'progressive'
    else:
        return 'other'

def plot_feature_combined_individual(results, param_val, save_dir, overall_title, skip_initial_rounds=5):
    """Create 1x3 plots for feature/combined heterogeneity individual experiments"""
    ...
    # Plot 1: Parameter Convergence, Parameter Error, Gradient Convergence
    ...
    ax3 = axes[2]
    sum_gradient_norms = data['sum_gradients_norm_trajectory']
    sum_gradient_norms_plot = sum_gradient_norms[skip_initial_rounds:]
    ax3.plot(rounds_plot, sum_gradient_norms_plot, color='magenta', alpha=0.8, linewidth=1.5)
    ax3.set_title('Collective Gradient Norm')
    ax3.set_xlabel('Rounds')
    ax3.set_ylabel(r'$||\sum_{i=1}^{10} p^{(i)}h^{(i)}(\bar{w}_{nN})||$')
    ...
    # Plot 2: Parameter Convergence, Gradient Convergence, All Client Gradient Norm
    fig, axes = plt.subplots(1, 3, figsize=COMPARISON_1X3_SIZE)
    fig.suptitle(f'{overall_title}')
    ...
    # Gradient Convergence (repeated)
    ax2 = axes[1]
    ax2.plot(rounds_plot, sum_gradient_norms_plot, color='magenta', alpha=0.8, linewidth=1.5)
    ax2.set_title('Collective Gradient Norm')
    ax2.set_xlabel('Rounds')
    ax2.set_ylabel(r'$||\sum_{i=1}^{10} p^{(i)}h^{(i)}(\bar{w}_{nN})||$')
    ax2.grid(True, alpha=0.3)
    format_axis_decimals(ax2, 'gradient')
    
    # All Client Gradient Norm
    ax3 = axes[2]
    if 'all_clients_gradient_norm_trajectory' in data:
        ...
        for client_id in range(NUM_CLIENTS):
            ...
            ax3.plot(rounds_plot, all_gradient_norms_plot[:, client_id], 
                    label=rf'$\mathcal{{C}}^{{({c_id})}}$', color=color, alpha=0.7, linewidth=1.0)
...
def plot_parameter_target_taper_individual(results, param_val, save_dir, overall_title, skip_initial_rounds=5):
    """Create 1x2 plots for parameter/target/taper individual experiments"""
    if param_val not in results or 5 not in results[param_val]:
        return
    ...
    # Plot: Parameter Error, Gradient Convergence
    ...
    parameter_errors = data['parameter_error_trajectory']
    parameter_errors_plot = parameter_errors[skip_initial_rounds:]
    ax1.plot(rounds_plot, parameter_errors_plot, color='orange', alpha=0.8, linewidth=1.5)
    ax1.set_title('Parameter Error')
    ax1.set_xlabel('Rounds')
    ...
    configure_academic_plot(fig, axes)
    plt.tight_layout()
    plt.savefig(f"{save_dir}/{safe_filename}_analysis.png", dpi=ACADEMIC_DPI, bbox_inches='tight')
    plt.close()

def plot_feature_combined_comparative(results, parameter_values, save_dir, overall_title, skip_initial_rounds=5):
    """Create comparative 1x2 plots for feature/combined heterogeneity across experiments"""
    N = 5
...
def plot_parameter_target_taper_comparative(results, parameter_values, save_dir, overall_title, skip_initial_rounds=5):
    """Create comparative 1x2 plots for parameter/target/taper across experiments"""
    N = 5
    colors = plt.cm.viridis(np.linspace(0, 1, len(parameter_values)))
...
def plot_progressive_results_n5(results, save_dir, study_name, parameter_values, experiment_type, overall_title, skip_initial_rounds=5):
    """Main plotting function for N=5 progressive results with specific layouts"""
    print(f"Generating {study_name} plots for N=5 only (experiment type: {experiment_type})")
    ...
    for param_val in parameter_values:
        ...
        if experiment_type == 'feature_combined':
            plot_feature_combined_individual(results, param_val, individual_dir, overall_title, skip_initial_rounds)
        elif experiment_type == 'parameter_targetr':
            plot_parameter_target_taper_individual(results, param_val, individual_dir, overall_title, skip_initial_rounds)
        elif experiment_type == 'taper':
            plot_parameter_target_taper_individual(results, param_val, individual_dir, overall_title, skip_initial_rounds)
        else:
            print(f"    Skipping {label} - not a target experiment type")
    
    # Create comparative plots
    print("Creating comparative plots across experiments...")
    if experiment_type == 'feature_combined':
        plot_feature_combined_comparative(results, parameter_values, save_dir, overall_title, skip_initial_rounds)
    elif experiment_type == 'parameter_target_taper':
        plot_parameter_target_taper_comparative(results, parameter_values, save_dir, overall_title, skip_initial_rounds)
...
def determine_study_info(pickle_path):
    ...
    if 'progressive' in path.name.lower():
        ...
        study_name = "Progressive Study"
    
    return study_name

def main():
    parser = argparse.ArgumentParser(description='Plot N=5 Progressive results from pickle files')
    ...
    parser.add_argument('--output-dir', help='Output directory for plots (default: folder/plots_n5)')
    ...
    if args.output_dir:
        ...
        output_dir = os.path.join(args.folder, "plots_n5")
    ...
    for pickle_path in progressive_files:
        ...
        experiment_type = identify_experiment_type(pickle_path)
        
        print(f"Identified experiment type: {experiment_type}")
        
        if experiment_type not in ['feature_combined', 'parameter_target', 'taper']:
            print(f"Experiment type '{experiment_type}' not in target types, skipping...")
            continue
    ...
    print(f"\n{'='*60}")
    print("All N=5 progressive plotting completed!")
