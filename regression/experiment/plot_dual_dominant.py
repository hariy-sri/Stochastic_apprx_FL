...
def plot_dual_dominant_individual(results, param_val, save_dir, overall_title, skip_initial_rounds=5):
    """Create individual plots for dual dominant experiments"""
    ...
    # Plot 1: Parameter Convergence, Parameter Error, Combined Gradient Norm
    ...
    for comp in range(3):
        ax1.plot(rounds_plot, trajectory_plot[:, comp], 
                label=f'$w[{comp+1}]$', color=component_colors[comp], alpha=0.8, linewidth=1.5)
    ...
    ax3 = axes[2]
    combined_gradient_norms = data['combined_gradient_norm_trajectory']
    combined_gradient_norms_plot = combined_gradient_norms[skip_initial_rounds:]
    ax3.plot(rounds_plot, combined_gradient_norms_plot, color='magenta', alpha=0.8, linewidth=1.5)
    ax3.set_title('Combined Gradient Norm')
    ax3.set_xlabel('Rounds')
    ax3.set_ylabel(r'$||h^{(1)}(\bar{w}_{nN}) + 0.5h^{(2)}(\bar{w}_{nN})||$')
    ...
    # Plot 2: Parameter Convergence, Combined Gradient Norm, All Client Gradient Norms
    fig, axes = plt.subplots(1, 3, figsize=COMPARISON_1X3_SIZE)
    fig.suptitle(f'{overall_title}')
    ...
    # Combined Gradient Norm (repeated)
    ax2 = axes[1]
    ax2.plot(rounds_plot, combined_gradient_norms_plot, color='magenta', alpha=0.8, linewidth=1.5)
    ax2.set_title('Combined Gradient Norm')
    ax2.set_xlabel('Rounds')
    ax2.set_ylabel(r'$||h^{(1)}(\bar{w}_{nN}) + 0.5h^{(2)}(\bar{w}_{nN})||$')
    ax2.grid(True, alpha=0.3)
    ...
    if 'all_clients_gradient_norm_trajectory' in data:
        ...
        for client_id in range(NUM_CLIENTS):
            ...
            if client_id == 0:
                alpha, linewidth, label = 1.0, 2.0, r'$\mathcal{{C}}^{(1)}$'
            elif client_id == 1:
                alpha, linewidth, label = 1.0, 2.0, r'$\mathcal{{C}}^{(2)}$'
            else:
                c_id = client_id + 1
                alpha, linewidth, label = 0.6, 1.0, rf'$\mathcal{{C}}^{{({c_id})}}$'
...
def plot_dual_dominant_comparative(results, parameter_values, save_dir, overall_title, skip_initial_rounds=5):
    """Create comparative 1x2 plots for dual dominant across experiments"""
    N = 5
    colors = plt.cm.viridis(np.linspace(0, 1, len(parameter_values)))
    
    # Comparative Plot: Parameter Error, Combined Gradient Norm
    ...
    # Combined Gradient Norm Comparison
    ax2 = axes[1]
    for param_idx, param_val in enumerate(parameter_values):
        if param_val in results and N in results[param_val]:
            combined_gradient_norms = results[param_val][N]['combined_gradient_norm_trajectory']
            combined_gradient_norms_plot = combined_gradient_norms[skip_initial_rounds:]
            rounds_plot = np.arange(skip_initial_rounds + 1, len(combined_gradient_norms) + 1)
            label = get_distribution_label(param_val)
            ax2.plot(rounds_plot, combined_gradient_norms_plot, 
                    label=label, color=colors[param_idx], alpha=0.8, linewidth=1.5)
    
    ax2.set_title('Combined Gradient Norm Comparison')
    ax2.set_xlabel('Rounds')
    ax2.set_ylabel(r'$||h^{(1)}(\bar{w}_{nN}) + 0.5h^{(2)}(\bar{w}_{nN})||$')
    ax2.grid(True, alpha=0.3)
    format_axis_decimals(ax2, 'gradient')
...
def plot_dual_dominant_results_n5(results, save_dir, study_name, parameter_values, overall_title, skip_initial_rounds=5):
    """Main plotting function for N=5 dual dominant results"""
    ...
    for param_val in parameter_values:
        ...
        plot_dual_dominant_individual(results, param_val, individual_dir, overall_title, skip_initial_rounds)
    
    # Create comparative plots
    print("Creating comparative plots across experiments...")
    plot_dual_dominant_comparative(results, parameter_values, save_dir, overall_title, skip_initial_rounds)
...
def determine_study_info(pickle_path):
    ...
    if 'progressive' in path.name.lower():
        study_name = "Progressive Dual Dominant Study"
    else:
        study_name = "Dual Dominant Study"
    
    return study_name

def main():
    parser = argparse.ArgumentParser(description='Plot N=5 Dual Dominant Progressive results from pickle files')
    ...
    parser.add_argument('--output-dir', help='Output directory for plots (default: folder/plots_dual_dominant_n5)')
    ...
    if args.output_dir:
        ...
        output_dir = os.path.join(args.folder, "plots_dual_dominant_n5")
    ...
    print(f"\n{'='*60}")
    print("All N=5 dual dominant progressive plotting completed!")
