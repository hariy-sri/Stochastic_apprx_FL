...
TAPERING_EXPONENTS = [3.0/4.0, 3.3/4.0, 3.6/4.0, 3.9/4.0, 4.0/4.0]
...
def load_client_data_with_metadata(dataset_path):
    ...
    true_theta = np.array(metadata['reference_theta'], dtype=np.float32)
    ...
    print(f"Using reference_theta: {true_theta}")
...
def process_client_stochastic_no_dominant(client_id, X, Y, theta, base_lr, batch_counter, local_steps, round_num, lr_exponent):
    """Process N local steps for a client using stochastic method with uniform tapering"""
    all_gradients = []
...
def run_stochastic_simulation_no_dominant(clients_data, true_theta, lr_exponent, dataset_name=None):
    """Run stochastic simulation for all N values with no dominant clients"""
    print(f"Running No Dominant Stochastic simulation with LR exponent={lr_exponent:.3f}" + 
          (f" on {dataset_name}" if dataset_name else ""))
    
    print(f"Reference theta: {true_theta}")
    ...
    for N in N_VALUES:
        ...
        sum_gradients_trajectory = []
        sum_gradients_norm_trajectory = []
        ...
        for r in range(ROUNDS):
            ...
            for client_id in range(NUM_CLIENTS):
                ...
                base_lr = STOCHASTIC_BASE_LR
                
                theta, gradient, new_batch_counter = process_client_stochastic_no_dominant(
            ...
            sum_gradient = np.sum(round_gradients, axis=0)
            sum_gradients_trajectory.append(sum_gradient)
            sum_gradients_norm_trajectory.append(np.linalg.norm(sum_gradient))
        ...
        final_sum_gradient = np.sum([final_round_gradients[i] for i in range(NUM_CLIENTS)], axis=0)
        final_sum_gradient_norm = np.linalg.norm(final_sum_gradient)
        
        final_individual_gradient_norms = [np.linalg.norm(final_round_gradients[i]) for i in range(NUM_CLIENTS)]
        ...
        results[N] = {
            ...
            'sum_gradients_trajectory': sum_gradients_trajectory,
            'sum_gradients_norm_trajectory': sum_gradients_norm_trajectory,
            'global_loss_trajectory': global_loss_trajectory,
            'final_sum_gradient_norm': final_sum_gradient_norm,
            'final_individual_gradient_norms': final_individual_gradient_norms,
            'final_global_theta': final_global_theta,
            'final_parameter_error': final_parameter_error,
            'true_theta': true_theta
        }
        
        print(f"    Final sum gradient norm: {final_sum_gradient_norm:.6f}")
        print(f"    Final parameter error: {final_parameter_error:.6f}")
        avg_individual_norm = np.mean(final_individual_gradient_norms)
        print(f"    Average individual gradient norm: {avg_individual_norm:.6f}")
    
    return results


def run_heterogeneity_study_no_dominant():
    """Run heterogeneity ablation study for all dataset types with no dominant clients"""
    print("Starting No Dominant Heterogeneity Ablation Study...")
    
    base_save_dir = "heterogeneity_study_no_dominant"
        ...
        if config['progressive']:
            ...
            for dataset_name in config['progressive']:
                ...
                try:
                    ...
                    results = run_stochastic_simulation_no_dominant(clients_data, true_theta, lr_exponent, dataset_name)
    ...
    print(f"\nNo Dominant Heterogeneity study completed! Results saved in {base_save_dir}/")
    return base_save_dir

def run_tapering_study_no_dominant():
    """Run tapering ablation study with no dominant clients"""
    print("Starting No Dominant Tapering Ablation Study...")
    
    save_dir = "tapering_study_no_dominant"
    ...
    for lr_exponent in TAPERING_EXPONENTS:
        ...
        results = run_stochastic_simulation_no_dominant(clients_data, true_theta, lr_exponent)
        tapering_results[lr_exponent] = results
    
    with open(f"{save_dir}/tapering_results_no_dominant.pkl", "wb") as f:
        pickle.dump(tapering_results, f)
    print(f"Results saved to {save_dir}/tapering_results_no_dominant.pkl")
...
if __name__ == "__main__":
    print("Starting Enhanced No Dominant Ablation Studies for Stochastic Method with Heterogeneity Datasets")
    print("=" * 105)
    
    if sys.stdin.isatty():
        print("Select study to run:")
        print("1. Heterogeneity Study (All types - No Dominant)")
        print("2. Tapering Study (No Dominant)")
    ...
    if choice == "1":
        run_heterogeneity_study_no_dominant()
    elif choice == "2":
        run_tapering_study_no_dominant()
    elif choice == "3":
        run_heterogeneity_study_no_dominant()
        print("\n" + "=" * 105)
        run_tapering_study_no_dominant()
    ...
    print("\nAll enhanced no dominant ablation studies completed successfully!")